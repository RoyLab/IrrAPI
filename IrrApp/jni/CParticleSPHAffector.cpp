...
//! constructor
CParticleSPHAffector::CParticleSPHAffector(
...
{
	#ifdef _DEBUG
	setDebugName("CParticleSPHAffector");
	#endif
}


//! Affects an array of particles.
void CParticleSPHAffector::affect(u32 now, SParticle* particlearray, u32 count)
{
	if( LastTime == 0 )
	{
		LastTime = now;
	...
	{
		core::vector3df tmpPo = core::vector3df(Point.X,particlearray[i].pos.Y,Point.Z);
		core::vector3df direction = (tmpPo - particlearray[i].pos).normalize();
		direction *= -Speed * timeDelta;

		LOGE("DIRX:%f",direction.X);
