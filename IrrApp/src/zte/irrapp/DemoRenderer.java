...
public class DemoRenderer implements Renderer {
	...
	Scene scene = engine.getScene();
		...
		//BoundingBox bbox = models.getAbsoluteBoundingBox();
	}
	...
	public void onCreate(Engine engine) {      
		...
		for(int i=0;i<5;++i)
			star[i].setVisible(false);	//	五环
		ps.setVisible(false);			//火焰	
		cube.setVisible(false);		//平移立方体
		walla.setVisible(false);		//左侧墙体
		wallb.setVisible(false);		//右侧墙体
		bill.setVisible(false);			//公告板
		ball.setVisible(true); 			//彗星球体
		psCT.setVisible(true);		//彗星拖尾
		psStars.setVisible(true);		//星空
		psExp.setVisible(true);		//爆炸
	...
	/*public void onCreate(Engine engine){
		...
		models = scene.addMeshSceneNode("models/axis.obj", origin, null);
		models.setBBoxVisibility(true);
	}*/
	...
	private MeshSceneNode cube,walla,wallb,ball,models;
